    ...
    while idx < num_samples:
        ...
        for episode_idx in range(episode_len):
            ...
            for batch_idx in range(batch_size):
                episode_states[batch_idx, :, :, episode_idx] = states[batch_idx]
                episode_actions[batch_idx, episode_idx] = actions[batch_idx] 
                episode_rewards[batch_idx, episode_idx] = rewards[batch_idx] 
        ...
        for batch_idx in range(batch_size):
            ...
            cnt_zeros = 0
            for episode_idx in range(episode_len):
                if episode_rewards[batch_idx, episode_idx] == 0:
                    cnt_zeros += 1

            if cnt_zeros < 6:

                if num_samples <= idx + batch_idx:
                    break

                buff_states[idx, :, :, :] = episode_states[batch_idx]
                buff_actions[idx, :] = episode_actions[batch_idx] 
                buff_rewards[idx] = episode_rewards[batch_idx]
