...
class ReplayMemory:
    def __init__(self, capacity: int, input_dims: tuple):
        ...
        self.states = np.load("./ReplayMemoryData/states_1500000.npy")#np.zeros((self.capacity, *input_dims), dtype=np.float32)
        self.actions = np.load("./ReplayMemoryData/actions_1500000.npy")#np.zeros(self.capacity, dtype=np.int32)
        self.next_states = np.load("./ReplayMemoryData/next_states_1500000.npy")#np.zeros((self.capacity, *input_dims), dtype=np.float32)
        self.rewards = np.load("./ReplayMemoryData/rewards_1500000.npy")#np.zeros(self.capacity, dtype=np.float32)
